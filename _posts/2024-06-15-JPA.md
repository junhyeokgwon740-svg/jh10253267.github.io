...
![image](https://github.com/user-attachments/assets/3bd3d821-315a-452d-8fda-d7f366f7f7fe)

**EntityManager**  
영속성 컨텍스트를 관리하는 객체로 Thread-Safe하지 않다. 따라서 하나의 스레드당 하나만 생성된다. 
**EntityManagerFactory**  
EntityManager를 생성하며 불변객체로 Thread-Safe하다는 특징이 있다.

DB작업이 필요한 순간 EntityManager를 생성한다. 이를 위해 DataSource와 매핑된 Persistence에서 커넥션을 얻어온다.  
EMF는 EM을 생성하고 EM은 커넥션을 지연 로딩으로 가지게 된다.  

EM이 생성되면 매핑된 EMF에서 커넥션을 얻지만 커넥션이 필요한 시점 Flush가 호출되는 시점에서 커넥션을 사용한다.

실제 작업이 시작될 때 EM은 EntityTransaction객체를 생성해 트랜잭션을 시작하며 커넥션을 연결한다.  
이후 작업이 끝나면 EntityTransaction객체를 폐기하고 트랜잭션을 종료하며 EM도 폐기한다.  
EM은 Thread-Safe하지 않기 때문에 매 작업마다 인스턴스를 새로 생성한다.

![image](https://github.com/user-attachments/assets/3b945866-49de-4eb6-8f58-b41b425921e2)

엔티티는 위와 같은 생명주기를 가진다.  

엔티티 객체를 생성하면 이는 단순히 자바 객체에 지나지 않는다. 따라서 DB와 동기화 되지 않는 상태이다. 이를 비영속이라 부른다.

영속 상태는 엔티티가 영속성 컨텍스트에 의해 관리되는 상태를 말한다.  
persist메소드를 통해 영속 상태에 들어가게 할 수 있다.

detach는 영속성 컨텍스트에 저장되었다가 분리된 상태로 더 이상 영속 상태의 엔티티가 아니다.  
detach : 엔티티를 영속성 컨텍스트에서 분리  
clear : 영속성 컨텍스트를 비움  
close : 영속성 컨텍스트를 종료

엔티티가 영속성 컨텍스트와 DB에서 삭제되도록 예약된 상태로 최종적으로 모두 삭제된다. remove메소드를 통해 이루어진다. 영속성 컨텍스트에서 삭제가 예약되고 이후 flush호출시 영속성 컨텍스트에서 제거됨과 동시에 데이터베이스에 delete쿼리가 나가게 된다.
