...
그러나 자바의 방식은 조금 다르다. 자바에서는 한 클래스의 필드로 다른 클래스를 소유하는 방식(참조하는 방식)으로 연관관계를 맺는다.  
그러므로 게시글이 댓글을 참조할 수도 있고 댓글이 게시글을 참조할 수도 있고 게시글이 댓글을 참조하고 댓글이 게시글을 참조하는 것도 가능하다.  
```java
@Data
public class Post {
...
개인적인 생각은 개발자에게 중요한 것은 쿼리를 작성하는 것이 아닌 비즈니스 로직을 고민하고 작성하는 것이라고 생각한다. 그렇기 때문에 이런 반복 작업에 시간과 체력이 소모되는 것은 좋지 않다고 생각하다.  
