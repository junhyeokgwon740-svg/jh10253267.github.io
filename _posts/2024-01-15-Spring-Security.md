...
유효한 Access token을 가지고있다면 인증된 사용자로 판단한다. 그러나 토큰 자체가 인증 정보로 이루어져있고 단순한 문자열이라는 특징, 토큰이 클라이언트 측에 보관된다는 점 때문에 탈취당할 위험이 있다. 따라서 Access token의 유효기간을 최대한 짧게 설정하고 Access token을 재발급받을 수 있는 Refresh token을 사용하여 인증처리를 하는 것이 일반적이다.  
...
신용카드를 잃어버렸을 때 카드를 중지시키고 새롭게 발급받는 것과 같은 발상이다.  
이러면 신용카드를 주웠다고 하더라도 카드만 있을 뿐이지 아무 쓸모가 없어진다.
