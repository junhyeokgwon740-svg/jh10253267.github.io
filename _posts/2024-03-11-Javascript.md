...
배열 안에는 모든 타입이 다 들어갈 수 있다. 배열안의 배열, 배열안의 함수, 배열안의 객체도 가능하다.
...
만약 함수의 파라미터가 변동되는 경우에는 어떻게 해야할까?  
```js
function sum(x1, x2) {
  ley y = x1 + x2
}
...
```
위와 같이 할 수가 있을 것이다. 그러나 이와 같이 모두 구현해야한다면 상당히 피로한 작업이 아닐 수 없다.  
그럴 땐 Rest Parameter를 사용한다.
...
지금까지 우리는 함수를 정의할 때 함수 선언식과 함수 표현식을 사용하여 함수를 구현했다.  
화살표 함수는 함수를 정의하는 새로운 방식으로 이를 활용하면 구문이 짧게 단축된다는 장점이 있다.

```js
const test = (name) => {return name};
//파라미터가 하나만 있을 경우 괄호를 생략할 수 있다.
const test = name => {return name};
//함수의 유일한 문장이 리턴이라면 중괄호를 생략할 수 있다.
const test = name => name;
```

일반적으로 문자열을 병합하기 위해서 더하기 기호를 사용하는데 예를 들면 다음과 같다.
```js
function sample() {
  console.log("Hello" + name);
}
```
템플릿 리터럴은 변수에 할당된 문자열을 하나의 문자열로 병합할 때 더하기를 사용하지 않고도 하나의 문자열로 만들 수 있게 해준다.
```js
function sample() {
  let name = "James";
  console.log(`Hello ${name}`);
